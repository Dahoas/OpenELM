...
class Policy:
    def __init__(self):
        ...
        self.key_dropped = False  # New variable to track if the key has been dropped
    ...
    def act(self, observation):
        ...
        if self.trying_to_drop_key:
            ...
            if observation['image'][3][5][0] == 1:  # Empty space
                self.trying_to_drop_key = False
                self.key_dropped = True  # Mark the key as dropped
        ...
        # Picking up the key only if it hasn't been dropped yet
        if not self.holding_key and not self.key_dropped and observation['image'][3][5][0] == 5:
    ...
    def update(self, observation, action, reward, next_observation):
        ...
        if action == 4:
            self.holding_key = False
            self.key_dropped = True  # Update key status when dropped
        if reward > 0:
